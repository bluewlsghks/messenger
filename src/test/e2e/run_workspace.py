"""Capture failures before the real browser context closes."""
import ast
import json
import pathlib
import traceback
...
def instrumented_page(self, *args, **kwargs):
    ...
    page.on('response', lambda response: record('http', {'status': response.status, 'url': response.url}) if response.status >= 400 else None)
    ...
    for index, page in enumerate(list(transport)):
        ...
        try:
            ...
            state = page.evaluate("""() => ({url:location.href,
                ...
                notices:window.__notifications?.map(n=>({title:n.title,options:n.options})),
                permissionRequests:window.__permissionRequests,
...
exec(compile(tree, 'src/test/e2e/workspace.py', 'exec'), {'__name__': '__main__', '_capture_failure': capture_failure})
